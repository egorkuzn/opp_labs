...
//Mask arrays:
const bool HORIZONTAL[2] = {false, true},
           VERTICAL  [2] = {true, false};
//Grid parameters box:
struct grid{
    int height,
        width,
...
void Grid_Init(int height, int width){
    grid.height = height;
    grid.width = width;
    grid.array[1] = height;
    grid.array[2] = width;
...
void Matrix_B_Scatter(MPI_Comm* COMM_2D, MPI_Datatype* B_TYPE, int* coords, int* root, double* B_part){
...
void final_free(double* A_part, double* B_part, double* C_part, MPI_Datatype* B_TYPE, MPI_Comm* COMM_HORIZONTAL, MPI_Comm* COMM_VERTICAL, MPI_Comm* COMM_2D){
    free(matrixes.A);
    free(matrixes.B);
    free(matrixes.C);
...
//New communicators creation:
    ...
    MPI_Cart_create(MPI_COMM_WORLD, 2, grid.array, periods, true, &COMM_2D);
...
//Taking part of matrix B: vvvvvvvvvvvvvvvvvvvvvvvvТУТ ЕЩЁ ЕСТЬ ВОЗМОЖНОСТЬ НАВЕСТИ КРАСОТУvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    ...
    int root;
    Matrix_B_Scatter(&COMM_2D, &B_TYPE, coords, &root, B_part);
...
//Memory deallocation:
    final_free(A_part, B_part, C_part, &B_TYPE, &COMM_HORIZONTAL, &COMM_VERTICAL, &COMM_2D);
//Final output:
    if(processRank == 0)
