...
void final_free(double* A, double* X, double* b, double* final_vect_res,
                ...
                double* piece, double* Axn_minus_b_buffer){
    free(A);
    free(X);
    free(b);                
    free(final_vect_res);
 ...
 */ 
    ...
    do {
        ...
        sub(final_vect_resBuf, b + N / sizeOfCluster * processRank * sizeof(double), Axn_minus_b_buffer, fixedCutVectorSize); // parallel diff calculating for [A*xn - b] (we getting pieces of that) 
